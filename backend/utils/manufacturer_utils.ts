...
// Note: If needed, the path must be set so that it leads to the main folder of 'org1.example.com'
const startingDir = path.resolve(__dirname, '..', '..', 'fabric-samples');
const cryptoPath = envOrDefault('CRYPTO_PATH', path.resolve(startingDir, 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com'));
