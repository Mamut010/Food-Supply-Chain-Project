...
// Note: If needed, the path must be set so that it leads to the main folder of 'org3.example.com'
const startingDir = path.resolve(__dirname, '..', '..', 'fabric-samples');
const cryptoPath = envOrDefault('CRYPTO_PATH', path.resolve(startingDir, 'test-network', 'organizations', 'peerOrganizations', 'org3.example.com'));
...
const certPath = envOrDefault('CERT_PATH', path.resolve(cryptoPath, 'users', 'User1@org3.example.com', 'msp', 'signcerts', 'cert.pem'));
