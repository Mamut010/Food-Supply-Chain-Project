...
const caHostnameList:any = {
    "Manufacturer": "ca-org1",
    "Distributor": "ca-org2",
    "Retailer": "ca-org3",
}

const walletFolder:any = {
    "Manufacturer": "walletManufacturer",
...
const affiliationList:any = {
    ...
    "Retailer": "Retailer.department1",
}

const roleList = ["Manufacturer", "Distributor", "Retailer"];
...
const isRetailer = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const {userRole: role} = await getRole(req.userId!== undefined?req.userId:'');
        if (role === 'Retailer') {
            next();
...
const getRole = async (userId:string) => {
    ...
    for (const role of roles) {
        ...
        if (role === "Manufacturer") {
            walletFolder = "walletManufacturer";
        }
